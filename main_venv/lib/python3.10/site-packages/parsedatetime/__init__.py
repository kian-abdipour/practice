...
class Calendar(object):
    ...
    def evalRanges(self, datetimeString, sourceTime=None):
        ...
        if rangeFlag in (1, 2):
            ...
            date = self.ptc.CRE_DATE3.get(endStr)
            ...
            if endYear is not None:
                ...
                date = self.ptc.CRE_DATE3.get(startStr)
            ...
            mth = self.ptc.CRE_DATE3.get(startStr)
    ...
    def _evalModifier(self, modifier, chunk1, chunk2, sourceTime):
        ...
        m = self.ptc.CRE_REMAINING.get(chunk2)
        if m is not None:
            index = m.start() + 1
            unit = chunk2[:m.start()]
            chunk2 = chunk2[index:]
    ...
    def _partialParseDateStr(self, s, sourceTime):
        ...
        if m is not None:
            ...
            if (m.group('date') != s):
                ...
                fTime = False
                mm = self.ptc.CRE_TIMEHMS2.get(s)
    ...
    def nlp(self, inputString, sourceTime=None, version=None):
        ...
        while startpos < len(inputString):
            ...
            m = self.ptc.CRE_MODIFIER.get(inputString[startpos:])
            if m is not None:
            ...
            # Quantity + Units
            m = self.ptc.CRE_UNITS.get(inputString[startpos:])
            if m is not None:
                debug and log.debug('CRE_UNITS matched')
            ...
            # Quantity + Units
            m = self.ptc.CRE_QUNITS.get(inputString[startpos:])
            if m is not None:
                debug and log.debug('CRE_QUNITS matched')
            ...
            m = self.ptc.CRE_DATE3.get(inputString[startpos:])
            # NO LONGER NEEDED, THE REGEXP HANDLED MTHNAME NOW
            ...
            # Standard date format
            m = self.ptc.CRE_DATE.get(inputString[startpos:])
            if m is not None:
                if leftmost_match[1] == 0 or \
                        leftmost_match[0] > m.start('date') + startpos:
                    leftmost_match[0] = m.start('date') + startpos
            ...
            # Natural language day strings
            m = self.ptc.CRE_DAY.get(inputString[startpos:])
            if m is not None:
                if leftmost_match[1] == 0 or \
                        leftmost_match[0] > m.start() + startpos:
                    leftmost_match[0] = m.start() + startpos
            ...
            # Weekday
            m = self.ptc.CRE_WEEKDAY.get(inputString[startpos:])
            if m is not None:
            ...
            # Natural language time strings
            m = self.ptc.CRE_TIME.get(inputString[startpos:])
            if m is not None:
                if leftmost_match[1] == 0 or \
                        leftmost_match[0] > m.start() + startpos:
                    leftmost_match[0] = m.start() + startpos
            ...
            # HH:MM(:SS) am/pm time strings
            m = self.ptc.CRE_TIMEHMS2.get(inputString[startpos:])
            if m is not None:
                if leftmost_match[1] == 0 or \
            ...
            # HH:MM(:SS) time strings
            m = self.ptc.CRE_TIMEHMS.get(inputString[startpos:])
            if m is not None:
                if leftmost_match[1] == 0 or \
                        leftmost_match[0] > m.start('hours') + startpos:
                    leftmost_match[0] = m.start('hours') + startpos
            ...
            if len(matches) > 0 and matches[-1][3] == 0:
                m = self.ptc.CRE_UNITS_ONLY.get(inputString[startpos:])
